...
func GetProcesses() ([]ProcessInfo, error) {
	...
	for _, p := range processes {
		...
		go func(proc *process.Process) {
			...
			status := strings.Join(statusSlice, ", ") // Convert []string to string
...
// detectChanges compares the old and new process list and identifies events
func detectChanges(prev map[int32]ProcessInfo, current []ProcessInfo) []string {
	...
	for _, proc := range current {
		...
		if _, exists := prev[proc.PID]; !exists {
			events = append(events, fmt.Sprintf("🔵 New Process Started: %s (PID: %d)", proc.Name, proc.PID))
