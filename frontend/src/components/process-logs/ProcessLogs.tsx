  ...
  useEffect(() => {
    if (window.runtime) {
      window.runtime.EventsOn("process_log", (log: string) => {
        try {
          const processData: ProcessLog = JSON.parse(log); // Parse incoming log
          setProcesses((prevProcesses) => {
            // Prevent duplicate entries
            const existingProcessIndex = prevProcesses.findIndex(p => p.pid === processData.pid);
            if (existingProcessIndex !== -1) {
              // Update existing process entry
